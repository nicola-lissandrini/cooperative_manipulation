...
void MPCWrapper::initParams () {
	try {
		string myName = ros::this_node::getName ();
		boost::filesystem::path myNamePath(myName);
		string myNameCut = myNamePath.filename ().string ();
		nh.getParam ("mpc_wrapper_config_"+myNameCut,params);
		enabled = true;
...
void MPCWrapper::initROS ()
{
	...
	inputSub = nh.subscribe ((string)params["input_topic"], 10, inputCallback);
	enableSub = nh.subscribe ("/neo11/mpc/enable", 1, enableCallback);
	disableSub = nh.subscribe ("/neo11/mpc/disable", 1, disableCallback);

	controlPub = nh.advertise<OutputDataMsg> ((string)params["control_topic"], 1);
}

void MPCWrapper::enableCallback (const std_msgs::String &inputDataMsg) {
	mpcWrapper->setEnabled (inputDataMsg.data, true);
}

void MPCWrapper::disableCallback (const std_msgs::String &inputDataMsg) {
	mpcWrapper->setEnabled (inputDataMsg.data, false);
}
...
MPCWrapper::MPCWrapper ()
{
	initParams ();
	initROS ();
	ROS_INFO ("%s being with %lg", ros::this_node::getName ().c_str (), (double) params["mpc_config"]["W"][0]);
...
void MPCWrapper::setEnabled(const string &val, bool enableMsg) {
	string myName = ros::this_node::getName ();

	if (val == myName) {
		ROS_INFO ("I'm %s and Ill be %d", myName.c_str (), enableMsg);
		enabled = enableMsg;
	} else {
		ROS_INFO ("I'm %s and this is not for me", myName.c_str (), enableMsg);
	}
}
