...
void MPC::solverGenerateOutput (int &solverStatus, int &sumIter, double &kkt)
{
	...
	outputData.cpuTime = -1; // not implemented
	outputData.nIter = sumIter;
	outputData.objVal = acado_getObjective ();

	cout << "Current control" << endl;
	cout << outputData.controlTraj.col (0) << endl;
	cout << "End state" << endl;
	cout << outputData.stateTraj.col (ACADO_N) << endl;
	cout << "OBj val " << endl;
	cout << outputData.objVal << endl;
}
...
# endif // ACADO_NXA
	...
	solverGenerateOutput (solverStatus, sumIter, kkt);
}
